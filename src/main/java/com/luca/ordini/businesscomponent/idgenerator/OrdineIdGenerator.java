...
public class OrdineIdGenerator implements IdGeneratorInterface, DAOConstansts{
	...
	public long getNextId() throws DAOException, ClassNotFoundException, IOException {
		...
		try {
			...
			rs = stmt.executeQuery(SELECT_ORDINE_SEQ); //Recupero il prossimo valore della sequenza proggressivo
			rs.next();
			id = rs.getLong(1);
